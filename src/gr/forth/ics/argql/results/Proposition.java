...
public class Proposition extends ResultValue {
	...
	public Proposition(String ID, String text) {
		super(ResultType.Proposition);
		this.ID = ID.replace("http://www.aifdb.org/nodes/", "");
		if (SPARQLTranslator.translationMode == TranslationMode.Normal) {
			this.value = text;
		} else {
			try { 
				//String uri = "http://www.aifdb.org/nodes" + ID;
				String query = "select * from equivalences where uri=\"" + ID + "\" and canonicalelement=\"" + text
						+ "\";";
			//	System.out.println("\n==>>>>" + query);
