...
import gr.forth.ics.argql.results.Path;
...
public class PathPattern {
	...
	/**
	 * This method implements the (p)*n case in the path pattern expressions. 
	 * @param path the path pattern to be multiplied
	 * @param num the multiplier number.
	 * @return the result path pattern of the multiplication.
	 */
	public static PathPattern multiplyPathWith(PathPattern path, int num) {

		PathPattern newPathPattern = new PathPattern();

		//copy the old relSequences in a new ArrayList
		...
		// generation of a new argument pattern that will be 
	...
	public String printPath() {
		...
		for (RelationPattern rel : relSequence) {
			...
			} else if (rel.getName().compareTo("Undermine") == 0) {
				UnderminePattern undermine = (UnderminePattern) rel;
				pathStr.append(" " + undermine.getName() + "(" + undermine.getCaVar() + ") ");
